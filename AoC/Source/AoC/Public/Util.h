...
namespace Util
{
	int64 StringToInt64(const FString& Str)
	{
		return FCString::Atoi64(*Str);
	}

	FString Int64ToString(int64 Number)
	{
		return FString::Printf(TEXT("%lld"), Number);
	}

	FString FileAsString(const FString& LocalPath)
	{
	...
	TArray<FString> FileAsLines(const FString& LocalPath)
	{
	...
	void selectInto(const TArray<_TIn> From, TArray<_TOut> & To, _Fn _Func) {
		To.SetNum(From.Num());
	...
	template <class _TIn, class _TOut, class _Fn>
	void selectInto(const _TIn * FromPtr, size_t FromSize, TArray<_TOut> & To, _Fn _Func) {
		To.SetNum(FromSize);
		std::transform(FromPtr, FromPtr + FromSize, To.GetData(), _Func);
	}

	template <class _TIn, class _TOut, class _Fn>
	TArray<_TOut> select(const TArray<_TIn> From, _Fn _Func) {
		TArray<_TOut> To;
	...
	template <class _TIn>
	size_t maxIndex(const TArray<_TIn> From) {
		return std::max_element(From.GetData(), From.GetData() + From.Num()) - From.GetData();
	}

	template <class _TIn>
	size_t maxIndex(const TArray<_TIn> From, size_t startIndex, size_t endIndex) {
